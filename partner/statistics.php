<?php
  ...
  $daily_stats = DailyStatistics::query("SELECT `day`, $sum FROM _T_ WHERE `day` >= DATE_ADD(NOW(), INTERVAL -1 MONTH) AND `partner_id`='%s' GROUP BY `day` ORDER BY `day`", $current_user->id);
  
  $test_stats = DailyStatistics::query("SELECT `test_id`, $sum FROM _T_ WHERE `day` >= DATE_ADD(NOW(), INTERVAL -1 MONTH) AND `partner_id`='%s' GROUP BY `test_id`", $current_user->id);
  $tests = Test::query_indexed('id', "SELECT `id`, `name` FROM _T_ WHERE `id` IN %s", collect_attrs($test_stats, 'test_id'));
